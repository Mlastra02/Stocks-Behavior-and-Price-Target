...
class TrendAndVolumeTest(unittest.TestCase):
    ...
    def test_since_year_filters_older_reports(self):
        prices = _build_dated_price_series(start="2018-01-01", periods=1500)
        records = [
            {"report_date": "2018-06-08", "report_hour": 16, "eps_actual": 1.0, "eps_estimate": 1.0, "surprise_pct": 0.0},
            {"report_date": "2023-06-08", "report_hour": 16, "eps_actual": 1.0, "eps_estimate": 1.0, "surprise_pct": 0.0},
        ]

        result = self._analyze_with(prices, records, since_year=2022)

        self.assertEqual(len(result.reactions), 1)
        self.assertEqual(result.reactions[0].report_date, "2023-06-08")

    def test_require_beat_filters_records_before_analysis(self):
        prices = _build_dated_price_series(start="2023-06-01", periods=300)
        records = [
            {"report_date": "2023-08-08", "report_hour": 16, "eps_actual": 1.1, "eps_estimate": 1.0, "surprise_pct": 0.10},
            {"report_date": "2023-11-08", "report_hour": 16, "eps_actual": 0.9, "eps_estimate": 1.0, "surprise_pct": -0.10},
        ]

        beats_only = self._analyze_with(prices, records, require_beat=True)
        self.assertEqual(len(beats_only.reactions), 1)
        self.assertGreater(beats_only.reactions[0].surprise_pct, 0)
